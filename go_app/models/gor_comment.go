...
func FindComment(id int64) (*Comment, error) {
	...
	var_comment := Comment{}
	err := db.Get(&var_comment, db.Rebind(`SELECT * FROM comments WHERE id = ? LIMIT 1`), id)
	if err != nil {
		log.Printf("Error: %v\n", err)
		return nil, err
	}
	return &var_comment, nil
}

// FirstComment find the first one comment by id ASC order
func FirstComment() (*Comment, error) {
	var_comment := Comment{}
	err := db.Get(&var_comment, db.Rebind(`SELECT * FROM comments ORDER BY id ASC LIMIT 1`))
	if err != nil {
		log.Printf("Error: %v\n", err)
		return nil, err
	}
...
func FirstComments(n uint32) ([]Comment, error) {
	...
	err := db.Select(&var_comments, db.Rebind(sql))
	if err != nil {
		log.Printf("Error: %v\n", err)
		return nil, err
	}
	return var_comments, nil
}

// LastComment find the last one comment by id DESC order
func LastComment() (*Comment, error) {
	var_comment := Comment{}
	err := db.Get(&var_comment, db.Rebind(`SELECT * FROM comments ORDER BY id DESC LIMIT 1`))
	if err != nil {
		log.Printf("Error: %v\n", err)
		return nil, err
	}
...
func FindComments(ids ...int64) ([]Comment, error) {
	...
	err := db.Select(&var_comments, sql, idsT...)
	if err != nil {
		log.Printf("Error: %v\n", err)
		return nil, err
	}
...
func FindCommentBy(field string, val interface{}) (*Comment, error) {
	...
	err := db.Get(&var_comment, db.Rebind(sqlStr), val)
	if err != nil {
		log.Printf("Error: %v\n", err)
		return nil, err
	}
...
func FindCommentsBy(field string, val interface{}) (var_comments []Comment, err error) {
	sqlFmt := `SELECT * FROM comments WHERE %s = ?`
	sqlStr := fmt.Sprintf(sqlFmt, field)
	err = db.Select(&var_comments, db.Rebind(sqlStr), val)
	if err != nil {
		log.Printf("Error: %v\n", err)
		return nil, err
	}
...
func CommentIncludesWhere(assocs []string, sql string, args ...interface{}) (var_comments []Comment, err error) {
	...
	if len(var_comments) <= 0 {
		return nil, errors.New("No results available")
	}
	ids := make([]interface{}, len(var_comments))
	for _, v := range var_comments {
...
// CommentIds get all the Ids of Comment records by where restriction
...
func (var_comment *Comment) CreateArticle(am map[string]interface{}) error {
	am["comment_id"] = var_comment.Id
	_, err := CreateArticle(am)
	return err
}

func (var_comment *Comment) Destroy() error {
	if var_comment.Id == 0 {
		return errors.New("Invalid Id field: it can't be a zero value")
	}
	err := DestroyComment(var_comment.Id)
	return err
}
...
func (var_comment *Comment) Save() error {
	ok, err := govalidator.ValidateStruct(var_comment)
	if !ok {
		errMsg := "Validate Comment struct error: Unknown error"
		if err != nil {
	...
	var_comment.UpdatedAt = time.Now()
	sqlFmt := `UPDATE comments SET %s WHERE id = %v`
	sqlStr := fmt.Sprintf(sqlFmt, "commenter = :commenter, body = :body, article_id = :article_id, updated_at = :updated_at", var_comment.Id)
	_, err = db.NamedExec(sqlStr, var_comment)
	return err
}
...
func (var_comment *Comment) Update(am map[string]interface{}) error {
	if var_comment.Id == 0 {
		return errors.New("Invalid Id field: it can't be a zero value")
	}
	err := UpdateComment(var_comment.Id, am)
	return err
}

func (var_comment *Comment) UpdateAttributes(am map[string]interface{}) error {
	if var_comment.Id == 0 {
		return errors.New("Invalid Id field: it can't be a zero value")
	}
	err := UpdateComment(var_comment.Id, am)
...
func (var_comment *Comment) UpdateColumns(am map[string]interface{}) error {
	if var_comment.Id == 0 {
		return errors.New("Invalid Id field: it can't be a zero value")
	}
	err := UpdateComment(var_comment.Id, am)
